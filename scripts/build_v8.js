...
const paths = {
	...
	deps: resolve(PKGS),
	depInclude: resolve(PKGS, "includes"),
	depLibs: resolve(PKGS, "libs")
};
...
const genLibPaths = () =>
{
	...
	mkdirp(paths.deps);
	mkdirp(paths.depInclude);
	mkdirp(paths.depLibs);
	for (const o of oss)
	{
		mkdirp(paths.depLibs, o);
		for (const p of platforms)
		{
			mkdirp(paths.depLibs, o, p);
			for (const t of buildTypes)
			{
				mkdirp(paths.depLibs, o, p, t);
			}
		}
	}
}
